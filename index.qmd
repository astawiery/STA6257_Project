...
covar_data <- respiratory
covar_data$sex <- as.numeric(covar_data$sex)
covar_data$treat <- as.numeric(covar_data$treat)


# co-variance matrix
covar_matrix <- select(covar_data, 1:7) %>% 
  cov()
covar_matrix
...
The Generalized Linear Model, shown as the glm() function above, does not account for the between segment correlations in longitudinal data. For this example, the coefficient estimates 
