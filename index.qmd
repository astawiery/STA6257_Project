...
resp_1g <- respiratory %>% group_by(visit)

g <- ggplot(resp_1g, aes(x = visit, group = outcome, fill = outcome))
g+geom_bar()+
  labs(x="visit number",y="Respiratory Health, '1' good, '0' poor")

resp_long <- pivot_longer(respiratory,cols = visit, names_to="visit_number",values_to="res_health"
) %>%
  mutate(
    visit_number = fct_inorder(paste("visit number", respiratory$visit)),
    res_health = respiratory$outcome
    )

ggplot(resp_long, aes(visit_number,res_health,fill=visit_number))+
  geom_boxplot()+
  geom_jitter(width=0.2)+
  guides(fill="none")+
  labs(x="",y="Respiratory Health, '1' good, '0' poor")
